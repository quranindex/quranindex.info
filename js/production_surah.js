...
QI.globals = {
	...
	accentMap: {
    	'â':'a','à':'a','Â':'A','À':'A','ç':'c','é':'e','è':'e','ê':'e','É':'E','È':'E','Ê':'E','í':'i','ó':'o','ú':'u','ü':'u','أ':'ا','إ':'ا','آ':'ا'
	},
    ...
    var
        utils = (function () {
            return {
                ...
                //     if(isSurah){
                //         q = q.replace(/^(s(o?u|o)ra[th]?e?|سورة)\s/gi,''); // sura words at beginning
                //         q = q.replace(/\s/gi,'[-]'); // highlight dashes only for surah
                //     }
                //     q = q.replace(/e/gi,'[eéèêëEÉÈÊË]');
                //     q = q.replace(/a/gi,'[aàâäAÀÁÂÃÄÅÆ]');
                //     q = q.replace(/c/gi,'[cçC]');
                //     q = q.replace(/i/gi,'[iïîIÌÍÎÏ]');
                //     q = q.replace(/o/gi,'[oôöÒÓÔÕÖ]');
                //     q = q.replace(/u/gi,'[uüûUÜÛÙÚ]');
                //     q = q.replace(/y/gi,'[yYÿ^yÝ]');
    ...
    Autocomplete.prototype = {
        ...
        isExactMatch: function (query) {
            ...
            return (suggestions.length === 1 && 
                ...
                || suggestions[0].value.toLowerCase().replace(/ô/g, 'o') === queryLowerCase
                || suggestions[0].value.toLowerCase().replace(/â|à|á/g, 'a') === queryLowerCase
                || suggestions[0].value.toLowerCase().replace(/ï|î/g, 'i') === queryLowerCase
                || suggestions[0].value.toLowerCase().replace(/^ال/g, '') === queryLowerCase
                )
            )
...
$(window).on('load', function() {
        ...
        $('.drop-lang').on('click', '.list-lang', function() {
            dataLayer.push({'event': 'translation', 'title': $(this).attr('data-lang'), 'path': location.pathname});
        });
        ...
        $('.drop-reciter').on('click', '.list-reciter', function() {
            dataLayer.push({'event': 'reciter', 'title': $(this).attr('title'), 'path': location.pathname});
        });
        ...
        //         dataLayer.push({'event': 'modal'{ttle: , $(this).attr('data-target'), 'path': location.pathname});
        // });
        ...
        $(".footer-social").on('click', 'a', function() {
            dataLayer.push({'event': 'social_page', 'title': $(this).attr('title'), 'path': location.pathname});
        });
        ...
        $(".social-list").on('click', 'a', function() {
                dataLayer.push({'event': 'social_share', 'title': $(this).attr('title'), 'path': location.pathname});
        });
        ...
        $(".dropdown-display li:not(.nav-header)").children().not(".inner-dropdown").on('click', function() {
            dataLayer.push({'event': 'setting', 'select': $(this).attr('data-select-display'), 'path': location.pathname});
        });
        // FONT SIZE AND NB AYAT VIEW // TO FIX, ON CHANGE SHOULD UPDATE GA EVENT TOO
        // $(".bar").on('click', function() {
        //     $(this).attr("o{title: nchange", $(this).attr('onchange') + ";dataLayer.push({'event': 'setting', (this).attr("id")+": "+ayat_fontsize_out.value, 'path': location.pathname});
        // });
        ...
        $(".dropdown-weblang").on('click', 'a', function() {
                dataLayer.push({'event': 'language', 'lang': $(this).attr('hreflang'), 'path': location.pathname});
        });
        ...
        $(".pdf-list:not(.surah-list)").on('mouseover', "a[data-toggle='tooltip']", function() {
                dataLayer.push({'event': 'download', 'title': $(this).prev().attr("title"), 'path': location.pathname});
        });
        ...
        $("#alphabetTerms").on('click', 'a', function() {
            dataLayer.push({'event': 'topic', 'data': $(this).text(), 'path': location.pathname});
        });
        ...
        $(".surah-list").on('mouseover', 'a.icon-info', function() {
            dataLayer.push({'event': 'surah', 'data': $(this).text(), 'path': location.pathname});
        });
        ...
        $("#references-list").on('click', 'a', function() {
            dataLayer.push({'event': 'outbound', 'url': $(this).attr('href'), 'path': location.pathname});
        });
        ...
        $(".menu-toggle").on('click', function() { //MENU BUTTON
            dataLayer.push({'event': 'button_menu', 'data': $(this).attr('data-placement'), 'path': location.pathname});
        });
        $(".weblang-toggle").on('click', function() { //LANG BUTTON
            dataLayer.push({'event': 'button_lang', 'path': location.pathname});
        });
        $("#arkeyb").on('click', function() { //KEYBOARD BUTTON
            $(this).on('click', function() {
                dataLayer.push({'event': 'button_keyboard', 'path': location.pathname});
            });
        });
        $("#btn-search-topic").on('click', function() { //SEARCH BUTTON
            dataLayer.push({'event': 'button_search', 'path': location.pathname});
        });
        $("#show-more").on('click', function() { //SHOW MORE RESULTS BUTTON
            ...
            dataLayer.push({'event': 'button_showmore', 'data': JSON.stringify(data), 'path': location.pathname});
        });
        $("#alphabetTerms-nav").on('click', ' .key', function() { //ALPHA KEYS BUTTONS
            dataLayer.push({'event': 'button_alphakeys', 'title': $(this).text(), 'path': location.pathname});
        });
        ...
        if(QI.globals.pageType === 2){
            $(".audio-flat").on('click', 'a', function() { //PLAY AUDIO
                dataLayer.push({'event': 'surah_audio', 'title': $(this).parent().attr('src'), 'path': location.pathname});
            });
            $(".surah-container").on('click', '.surah-options a', function() { //SURAH OPTIONS
                ...
                if($(this).hasClass('reading-mode') || $(this).hasClass('tafsir-mode') || $(this).hasClass('scroll-mode')){
                    ...
                    dataLayer.push({'event': 'surah_options', 'info': info, 'path': location.pathname});
            });
            $(".jumbotron").on('click', '.col-xs-1', function() { //PREV-NEXT ARROWS SURAHS
                ...
                dataLayer.push({'event': 'surah_adjacents', 'arrowDirection': arrowDirection, 'path': location.pathname});
            });
            $("[data-toggle='popover']").on('shown.bs.popover', function (e) {
                $(".btn-tafsir-options").on('click', function(index) { //TAFSIR BUTTONS
                    ...
                    info += " " + $(".tafsir-text").attr("id");
                        dataLayer.push({'event': 'tafsir_options', 'info': info, 'path': location.pathname});
                });
            });
        }
        ...
        if(QI.globals.pageType === 1){
            $(".aya-options:not(.surah-options)").on('click', 'a.playtoggle', function() { //PLAY AUDIO
                    dataLayer.push({'event': 'verse_audio', 'id': $(this).attr('id'), 'path': location.pathname});
            });
            $(".aya-options:not(.surah-options)").on('click', 'a:not(.playtoggle)', function() { //VERSE OPTIONS
                    dataLayer.push({'event': 'verse_options', 'title': $(this).attr('title'), 'path': location.pathname});
            });
            $(".souratname").find("a").first().on('click', function() { //SURAH NAME
                    dataLayer.push({'event': 'surah_name_container', 'title': $(this).attr('title'), 'path': location.pathname});
            });
            $(".alert-success").on('click', 'a', function() { //SURAH NAME IN SUCCESS
                    dataLayer.push({'event': 'surah_name_alert', 'title': $(this).attr('href'), 'path': location.pathname});
            });
        }
});
...
(function ($) {
	$.fn.listnav = function (options) {
		...
		var letters_ar = ["_","ا","ب","ت","ث","ج","ح","خ","د","ذ","ر","ز","س","ش","ص","ض","ط","ظ","ع","غ","ف","ق","ك","ل","م","ن","ه","و","ي","-"];
		...
		return this.each(function () {
			...
			var counts = {}, allCount = 0,
				...
				if(opts.arabic){
					...
					letters = letters_ar;
					opts.initLetter = "ا";
					opts.noMatchText = "لا يوجد مصطلح لهذا الحرف";
				}
				if(QI.globals.lang=="fr")
					opts.noMatchText = "Pas de sujet trouvé";
			...
			function addLetterClass(firstChar, secondChar, thirdChar, subtChar, $el, isPrefix) {
				...
				if(firstChar=="ا" && secondChar=="ل" && firstChar==subtChar){ // arabic starting w alif n alam which we should ignore
					// console.log(firstChar,$el.text());
					firstChar = thirdChar;

					// check if second letter is alif w/ hmza then replace w/ alif
					if(thirdChar=="أ" || thirdChar=="إ" || thirdChar=="آ")
						firstChar = "ا";
				}
    ...
    onSearchEvents: function(){
        $( "#search_form" ).on('submit', function(e) {
            ...
            if(term.length > 0){
                ...
                term = term.replace(/^(s(o?u|o)ra[th]?e?|سورة)\s/i, '');
...
function secondsTimeSpanToMMSS(seconds) {
    ...
    var result_hour = "";
    if (hours > 0) {
        result_hour = (hours < 10 ? '0' + hours : hours) + ":";
    }
    return result_hour + (minutes < 10 ? '0' + minutes : minutes) + ":" + (seconds < 10 ? '0' + seconds : seconds); //zero padding on minutes and seconds
}
var timings = getSurahTiming();
var currentAyaNbPlaying = 1;

function getSurahTiming() {
    var res = [];
    $(".single-verse").each(function(index) {
        res.push($(this).attr("data-timing"));
    });
    return res;
}

function scrollToVerse(nb) {
    nb = (nb==0) ? 1 : nb; // highlight verse 1 instead of 0
    var surahNb = $("#search").attr("data-surah");
    $('html, body').animate({
        scrollTop: $('#aya_' + surahNb + '_' + nb).offset().top - 150
    },'fast');
}
// find index of closest ayat for highlights
...
// function highlightAya_working(currentTime, direction) {
//     // ADD: aya highlight timing, red first aya on play, then consecutive red
//     currentTime_ms = currentTime * 1000;
//     // j = sessionStorage.audioSurahPlayingAyaNb;
//     j = (direction=='left' || sessionStorage.audioSurahPlayingAyaNb == undefined) ? 0 : sessionStorage.audioSurahPlayingAyaNb;
//     // if (restart || sessionStorage.audioSurahPlayingAyaNb == undefined) {
//     //     j = 0;
//     // }
//     if(j==0){
//         $(".single-verse").removeClass("aya-reading")
//         $(".single-verse").eq(j).addClass("aya-reading");
//         sessionStorage.audioSurahPlayingAyaNb = j;
//     }

//     console.log("highlightAya", direction, timings[j], currentTime_ms, j);
//     // var closest = closestValue(timings, currentTime_ms);
//     // scrollToVerse(parseInt(closest) + 1);
//     for (var i = j, l = timings.length; i < l; i++) {
//         // console.log("inside highlightaya for", timings[i],currentTime_ms,i);
//         if (timings[i] <= currentTime_ms) {
//             // j++;
//             $(".single-verse").removeClass("aya-reading")
//             $(".single-verse").eq(i).addClass("aya-reading");
//             // j = i;
//             console.log("less than",i);
//             // keep track of current time so that if change of reciters is triggered, start back from where left off
//             sessionStorage.audioSurahPlayingAyaNb = i;
//         } 
//         else{
...
//         }
//         // else {
//         //     if (j == 0 && restart) { // FIX: when user is listening to aya 5 and go back to first aya, then restart highlighting to first aya instead of taking value from session storage
//         //         $(".single-verse").removeClass("aya-reading")
//         //         $(".single-verse").eq(j).addClass("aya-reading");
//         //         sessionStorage.audioSurahPlayingAyaNb = j;
//         //         // restart = 0;
//         //     } else{ // stop for loop to avoid going until end of timings array
//         //         console.log("[highlightAya] broke loop at",j);
//         //         break;
//         //     }
//         // }
//     }
...
function highlightAya(currentTime, direction) {
    ...
    if(direction=='right'){
        for (j, l = timings.length; j < l; j++) {
            if (timings[j] > currentTime_ms) {
                break;
            }
        }
    }
    j = (j==0) ? 1 : j; // highlight verse 1 instead of 0
    $(".single-verse").removeClass("aya-reading");
    $(".single-verse").eq(j-1).addClass("aya-reading");
    sessionStorage.audioSurahPlayingAyaNb = j;
}
// jQuery Extensions
(function(old) {
...
$(document).ready(function() {
    ...
    var audio_flat = $("audio.flat");
    audio_flat.seekChanged = 0;
    ...
    $(audio_flat).replaceWith(function() {
        ...
        replacementDiv.attr("data-slideout-ignore", "");
        replacementDiv.removeClass("flat");
        // replacementDiv.id = "surah-audio";
        var tooltipPlayPause = "";
        if (!QI.globals.isMobile) tooltipPlayPause = "onmouseover='$(this).tooltip(\"show\")' data-toggle='tooltip' data-placement='auto' data-trigger='hover ioshover' data-original-title='" + i18n.t('options.details.spacebar-audio') + "'";
        replacementDiv.append($("<a rel='nofollow' " + tooltipPlayPause + " class='icon icon-start' id='play-pause'/>"));
        replacementDiv.append($("<input type='range' id='seek' value='0'/>"));
        replacementDiv.append($('<i id="audio-tooltip" data-toggle="tooltip" data-placement="auto top" data-animation="false" data-trigger="manual" data-class="audio-tooltip" data-container=".audio-flat"/>'));
        replacementDiv.append($("<span id='duration'>00:00</span>"));
        return replacementDiv;
    });
    // });
    audio_flat = $(".audio-flat");
    // $.each(audio_flats, function(index, audio_flat)
    // {
    audio_flat.song = new Audio($(audio_flat).attr("src"));

    if(audio_flat.song.src.length > 0){
        $("#preloader").delay(350).fadeOut('slow'); // remove preloader to show surah while loading audio in background
    }

    // FIX: audio slider flickering to the original value and position when holding it
    ...
    audio_flat.song.addEventListener('timeupdate', function() {
        ...
        if (QI.globals.surahReading){
            var ayaHighlighted = ($(".aya-text").children(".aya-reading").index())/2;
            highlightAya(audio_flat.song.currentTime, audio_flat.direction);
            var currentSessionStorageAyaNb = parseInt(sessionStorage.audioSurahPlayingAyaNb)-1;

            // check if link contains hash when cominf from search page e.g. http://qi/surah/yunus#aya_10_75 should not scroll but rather show aya_10_75
            if(!window.location.hash && QI.globals.surahScroll && ayaHighlighted != currentSessionStorageAyaNb){
                ...
                scrollToVerse(currentSessionStorageAyaNb);
            }
        }
    ...
    $(audio_flat).find("#seek").bind("change", function() {
        // console.log("\nbind change", audio_flat.song.currentTime, $(this).val());
        audio_flat.direction = (audio_flat.song.currentTime >= $(this).val()) ? 'left' : 'right';
        audio_flat.song.currentTime  = $(this).val();
        // audio_flat.seekChanged = 1;
        // console.log("change",audio_flat.song.currentTime);
        // if (QI.globals.surahReading && !QI.globals.isChrome) highlightAya(audio_flat.song.currentTime, 1);
        $(audio_flat).find("#seek").attr("max", audio_flat.song.duration);
    });

    // FIX: if iOS, it doesnt take loadeddata event but rather progress
    var loadedEvent = (QI.globals.isIos) ? 'progress': 'loadeddata';

    // if previous reciter was playing, take off from where we stopped audio
    audio_flat.song.addEventListener(loadedEvent, function() {
        // get surah aya nb and time only if surah reading is available
        // otherwise reset session storage variables
        if (QI.globals.surahReading) {
            if (sessionStorage.audioSurahPlayingAyaNb===undefined && audio_flat.song.paused) {
                sessionStorage.audioSurahPlayingAyaNb = 1;
                // $(".aya-text").eq(1).addClass(".aya-reading")
            }
            if (sessionStorage.audioSurahPlayingAyaNb && audio_flat.song.paused) {
                // console.log("loadededata",audio_flat.song.currentTime,sessionStorage.audioSurahPlayingAyaNb,timings);
                audio_flat.song.currentTime = timings[sessionStorage.audioSurahPlayingAyaNb-1] / 1000;
                
                // if(QI.globals.surahScroll)
                //     scrollToVerse(parseInt(sessionStorage.audioSurahPlayingAyaNb));
                audio_flat.direction = 'right';
            }
            if (!sessionStorage.audioSurahNbPlaying) sessionStorage.audioSurahNbPlaying = $(".surah_container").attr("id").split("_")[1];
        } else {
            sessionStorage.clear();
        }
        return;
    });
    // moving surah player slider makes it animate like spotify
    // $("#seek").on("input change mousedown", function(e){
    //     $(".audio-flat #seek").addClass("slider-thumb-active");
    //     console.log('test',e);
    // });
    // $("#seek").on("input change mouseup", function(e){
    //     $(".audio-flat #seek").removeClass("slider-thumb-active");
    //     console.log('test1',e);
    // });
    $(audio_flat).find("#play-pause").on('click', function(event) {
        // FIX: if time is more than hour, then decrease player bar size for small size screen
        if (window.innerWidth <= 400 && audio_flat.song.duration > 3600) $(audio_flat).find("#seek").css("width", "70%");
        var play_pause = $(event.target);
        if (play_pause.hasClass("icon-start")) // Play the song
        {
            play_pause.removeClass("icon-start");
            play_pause.addClass("icon-stop");
        ...
        {
            play_pause.removeClass("icon-stop");
            play_pause.addClass("icon-start");
            audio_flat.song.pause();
        }
    });

    $("#seek").on('mousemove touchmove', function(e){
        var clientX = (e.touches != null) ? e.touches[0].pageX : e.clientX;
        ...
        if($('body').hasClass("scrolled")){
            windowScrollTop = 40; // offset the tooltip when placed at bottom a bit down because of cursor pointer hiding it
            placementTooltip = 'bottom';
        }

        $("#audio-tooltip").data('bs.tooltip').options.placement = placementTooltip;

        // bigger player shown
        var offsetTop = $(this).offset().top + windowScrollTop;
        if($(".audio-flat").css("position") == "fixed"){
        ...
        $("#audio-tooltip").attr('data-original-title', secondsTimeSpanToMMSS(position));
        $("#audio-tooltip").tooltip('show');
    });

    $("#seek").on('mouseenter touchstart', function(e){
        $("#audio-tooltip").tooltip('show');
    });

    $("#seek").on('mouseleave touchend', function(e){
        $("#audio-tooltip").tooltip('hide');
    });
});
/*
 * clipboard.js v2.0.1
...
// if(!QI.globals.isSafari || QI.globals.isChrome || QI.globals.isFirefox)
// getsurah("'.$surah.'");
$("#search").val($(".surah_container").data("souratname"));
$("#search").attr("data-search", $(".surah_container").data("souratname").toLowerCase());
$("#search").attr("data-surah", $(".surah_container").data("souratorder"));
setTimeout(function(){
    $("#container").masonry({
            columnWidth: 1,
            itemSelector: ".aya_container"
...
if($(".surah_container").data("timingset")){
    QI.globals.surahReading = false;
    $(".reading-mode").removeClass("reading-on");
    $(".reading-mode").addClass("reading-disabled");
    $(".reading-mode").css("cursor", "not-allowed");
    $(".reading-mode").find("span.icon-checkmark").remove();
    $(".reading-mode").find("span.icon-eye").css("color","#767676");
    setTimeout(function(){
        $(".reading-mode").attr("title",i18n.t("options.details.surah.reading-disabled"));
    },100);
}
...
$("body").keyup(function(e){
   ...
   if(e.keyCode == 32 && e.target.className.indexOf("form-control") != 0){
        $("#play-pause").click();
   }
});

// check if divehi selected, then load font
if("<?= $verses_lang_tpl ?>" == "dv"){
    $(".aya_container").css({
        "font-family": "divehi",
    });
}

// FIX: reposition footer to auto bottom for longer surah page than window viewport
if ($("body").height() > $(window).height()) {
    $("footer").css("bottom","auto");
    $(".jumbotron").css("margin-bottom", "20px");
}

// compare session storage surah nb with current one to decide if we reset saved info used when switching between reciters, see audio_flat.js
if(sessionStorage.audioSurahNbPlaying != $(".surah_container").attr("id").split("_")[1]){
    // console.log(sessionStorage.audioSurahNbPlaying, $(".surah_container").attr("id").split("_")[1]);
    sessionStorage.clear();
}

// FIX: set reciter to local storage version
// if(QI.functions.localData("u_reciter"))
...
$(window).scroll(function(e) {
    ...
    if ($(this).scrollTop() >= scroller_anchor && $('.audio-flat').css('position') != 'fixed' && $('.surah-options').css('position') != 'fixed')
    {    
        ...
        // make popover for additional surah details show on top when scrolled down
        // $("#popover-details-2").data('bs.popover').options.placement = "auto top";

        // Changing the height of the scroller anchor to that of scroller so that there is no change in the overall height of the page.
        $('.scroller_anchor').css('height', '15px');

        // make details popover show on top
        // $('.surah-options').find(".popover-container").popover('destroy');
        // $('.surah-options').find(".popover-container").popover({placement: 'top'});

    }
    else if ($(this).scrollTop() < scroller_anchor && $('.audio-flat').css('position') != 'relative' && $('.surah-options').css('position') != 'relative')
    {    // If the user has scrolled back to the location above the scroller anchor place it back into the content.

        // Change the height of the scroller anchor to 0 and now we will be adding the scroller back to the content.
        $('.scroller_anchor').css('height', '0px');

        // make popover for additional surah details show on bottom when scrolled up
        // $("#popover-details-2").data('bs.popover').options.placement = "auto bottom";

        // Change the CSS and put it back to its original position.
        $('body').removeClass('scrolled');

        // make details popover show on bottom
        // $('.surah-options').find(".popover-container").popover('destroy');
        // $('.surah-options').find(".popover-container").popover({placement: 'bottom'});
    }
});

// FIX: click inside popover tafsir dont make it disappear
/* http://stackoverflow.com/questions/11703093/how-to-dismiss-a-twitter-bootstrap-popover-by-clicking-outside/19638199#19638199 */
...
// zoom capabilities for tafsir text
$(document).on('click', '.btn-zoom', function(){
    var zoomValue = 0;
    var zoomMin = 16;
    var zoomMax = 24;
    if($(this).children().hasClass("glyphicon-zoomin")){
        zoomValue = 1;
    }
    else if($(this).children().hasClass("glyphicon-zoomout")){
        zoomValue = -1;
    }
...
// get tafsir text size from local storage before showing popover
$("[data-toggle='popover']").on('shown.bs.popover', function (e) {
    if(e.target.className == "single-verse"){
        $(".tafsir-text").css("fontSize", function(i, value) {
            return parseInt(QI.functions.localData("u_fontSizeTafsir"));
        });
    }
});

// FIX: change reciter from localstorage
// $(document).ready(function(){
//     console.log($(".audio-flat").attr("src"));
// });

// activate reading mode for surah
// $(".reading-mode").click(function(){
//     if(!$(this).hasClass("reading-disabled")){ // fix: dont show highlights for reciters that dont have audio timing data
//         if($(this).hasClass("reading-on")){
//             $(this).removeClass("reading-on");
//             $(".single-verse").removeClass("aya-reading");
//             $(".reading-mode span.icon-checkmark").remove();
//             QI.globals.surahReading = false;
//         }
//         else{
//             $(this).addClass("reading-on");
//             $(".reading-mode").append('<span class="icon icon-checkmark"></span>');
//             QI.globals.surahReading = true;
//         }
//     }
// });

// activate scroll and tafsir mode for surah
$(".mode-surah").on('click', function(){
    var mode = $(this).attr('class').match(/\w+-mode/g)[0].replace("-mode","");
    if(mode != "reading" || (mode == "reading" && !$(this).hasClass("reading-disabled"))){
        if($(this).hasClass(mode+"-on")){
            if(mode=="reading") $(".single-verse").removeClass("aya-reading");
            if(mode=="tafsir") $('.single-verse').popover('disable');
            $(this).removeClass(mode+"-on");
            $("."+mode+"-mode span.icon-checkmark").remove();
            QI.globals["surah"+mode.ucfirst()] = false;
        }
        else{
            $(this).addClass(mode+"-on");
            if(mode=="tafsir") $('.single-verse').popover('enable');
            $("."+mode+"-mode").append('<span class="icon icon-checkmark"></span>');
            QI.globals["surah"+mode.ucfirst()] = true;
        }
    }
});
...
// play individual verses in surah page
function playAudio(surah, aya){

    var audio = $("#audio_aya");
    var reciter = audio.data("reciter");
    var playButton = $("#aya_audio_"+aya+" span");
    if(audio[0].paused){
        audio.attr("src", QI.globals.everyAyahUrl + reciter + pad(surah, 3) + pad(aya, 3) +".mp3");
...
function notifyAudioPlaying(){
    if ('Notification' in window) {
            var notificationEvents = ['onclick', 'onshow', 'onerror', 'onclose'];
            var title;
            var options;
            var reciter_img = $(".dropdown-reciter").attr("data-selected-reciter");

            title = $("span.effect2").text();
            options = {
              body: $(".list-reciter.selected").find("a").text().trim(),
              tag: 'preset',
              icon: QI.globals.base_url + "img/reciters/" + reciter_img + ".jpg"
            };
